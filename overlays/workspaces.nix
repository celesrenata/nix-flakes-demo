final: prev:
{
  aws-workspacesOverride = prev.stdenv.mkDerivation (finalAttrs: with prev; {
    pname = "aws-workspaces";
    version = "2024.2.4935";
  
    src = fetchurl {
      # Check new version at https://d3nt0h4h6pmmc4.cloudfront.net/ubuntu/dists/focal/main/binary-amd64/Packages
      urls = [
        "https://d3nt0h4h6pmmc4.cloudfront.net/ubuntu/dists/focal/main/binary-amd64/workspacesclient_${finalAttrs.version}_amd64.deb"
        "https://archive.org/download/workspacesclient_${finalAttrs.version}_amd64/workspacesclient_${finalAttrs.version}_amd64.deb"
      ];
      hash = "sha256-DcfHC37cFmHSmCrBjdgeHVRegKtC9309+ymS7pgh8zo=";
    };
  
    nativeBuildInputs = [
      autoPatchelfHook
      makeWrapper
      wrapGAppsHook
    ];
  
    # Crashes at startup when stripping:
    # "Failed to create CoreCLR, HRESULT: 0x80004005"
    dontStrip = true;
    enableSetupHook = true; 
    buildInputs = [
      stdenv.cc.cc.lib
      libkrb5
      curl
      lttng-ust
      libpulseaudio
      gtk3
      glib
      glib.dev
      dconf
      at-spi2-atk
      openssl_1_1.out
      icu66
      webkitgtk
      librsvg
      gdk-pixbuf
      libsoup
      glib-networking
      gsettings-desktop-schemas 
      graphicsmagick_q16
      hiredis
      libusb1
      libva
      libvdpau
      xorg.xcbutil
      pcsclite
    ];
  
    unpackPhase = ''
      ${dpkg}/bin/dpkg -x $src $out
    '';
  
    preFixup = ''
      patchelf --replace-needed liblttng-ust.so.0 liblttng-ust.so $out/usr/lib/x86_64-linux-gnu/workspacesclient/pcoip/libcoreclrtraceptprovider.so
      patchelf --replace-needed libGraphicsMagick++-Q16.so.12 libGraphicsMagick++.so.12 $out/usr/lib/x86_64-linux-gnu/pcoip-client/vchan_plugins/libvchan-plugin-clipboard.so
      patchelf --replace-needed libhiredis.so.0.14 libhiredis.so $out/usr/lib/x86_64-linux-gnu/workspacesclient/pcoip/libpcoip_core.so
    '';
  
    installPhase = ''
      runHook preInstall
      mkdir -p $out/bin $out/lib $out/share
  
      wrapProgram $out/usr/bin/workspacesclient \
        --prefix LD_LIBRARY_PATH : "${lib.makeLibraryPath finalAttrs.buildInputs}" \
        --set GDK_PIXBUF_MODULE_FILE "${librsvg.out}/lib/gdk-pixbuf-2.0/2.10.0/loaders.cache" \
        --set GIO_EXTRA_MODULES "${glib-networking.out}/lib/gio/modules" \
        --set GSETTINGS_SCHEMA_DIR "$out/usr/share/glib-2.0/schemas"
      cp ${rigsofrods-bin}/share/rigsofrods/lib/libwebp.so.6 $out/usr/lib/
      cp ${rigsofrods-bin}/share/rigsofrods/lib/libjbig.so.0 $out/usr/lib/
      mv $out/usr/bin/workspacesclient $out/bin
      runHook postInstall
    '';
    postInstall = ''
      ${glib.dev}/bin/glib-compile-schemas $out/usr/share/glib-2.0/schemas
    '';

    meta = with lib; {
      description = "Client for Amazon WorkSpaces, a managed, secure Desktop-as-a-Service (DaaS) solution";
      homepage = "https://clients.amazonworkspaces.com";
      license = licenses.unfree;
      mainProgram = "workspacesclient";
      maintainers = with maintainers; [ mausch dylanmtaylor ];
      platforms = [ "x86_64-linux" ]; # TODO Mac support
      sourceProvenance = with sourceTypes; [ binaryNativeCode ];
    };
  });
}
